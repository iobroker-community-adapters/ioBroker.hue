...
class Hue extends utils.Adapter {
        ...
        if (obj) {
            switch (obj.command) {
                ...
                case 'createUser': {
                    ...
                    if (obj.callback) {
                        if (res.error === 0) {
                            this.sendTo(obj.from, obj.command, { native: { user: res.message }, saveConfig: true }, obj.callback);
                        }
     ...
     * @param update the update sent by bridge
     */
    async handleContactSensorUpdate(update) {
     ...
     * @param update the update sent by bridge
     */
    async handleDevicePowerUpdate(update) {
        if (!update.power_state) {
            return;
        }
        const deviceId = update.owner.rid;
